...
class AddFrame(Frame):  # 继承Frame类
    ...
    def createPage(self):
        Label(self, text='上传书籍', font='SimSum -16 bold').grid(row=0, column=1, pady=10)
        ttk.Label(self, text=' 书籍名称:', font='SimSun -14').grid(row=1, column=0, pady=10)
        ttk.Label(self, text=' 出版社名称:', font='SimSun -14').grid(row=2, column=0, pady=10)
        ttk.Label(self, text=' 书籍类型:', font='SimSun -14').grid(row=3, column=0, pady=10)
        ttk.Label(self, text=' 出售价格:', font='SimSun -14').grid(row=4, column=0, pady=10)
        ttk.Label(self, text=' 书籍状态:', font='SimSun -14').grid(row=5, column=0, pady=10)
        ...
        ttk.Radiobutton(self, text="未出售", value=0, variable=self.is_enter).grid(row=5, column=1, padx=5, pady=10)
        ttk.Radiobutton(self, text="已出售", value=1, variable=self.is_enter).grid(row=5, column=2, padx=5, pady=10)
        self.is_enter.set(0)
        ...
        ttk.Button(self, text='提交', command=self.datacommit, width=10, bootstyle="Primary").grid(row=7, column=1,pady=10)
    ...
    def datacommit(self):
        ...
        if self.patient_name.get():
            ...
            showinfo(title='提示', message='书籍名称不能为空！')
...
class SearchFrame(Frame):  # 继承Frame类
    ...
    def createPage(self):
        Label(self, text='查询书籍信息', font='SimSum -16 bold').grid(row=0, column=1, pady=10)
        Button(self, text="显示全部书籍", command=self.displayall, font='SimSun -14').grid(row=1, column=4, padx=5, pady=10,
                                                                                     sticky=W + E + N + S)
        ...
        Label(self, text='结果:', font='SimSun -14').grid(row=4, column=0, padx=5, pady=10, sticky=N)
    ...
    def displayall(self):
        ...
        def all(event):
            ...
            if item:
                ...
                ms = ConnDB(as_dict=False)
                sql = "select * from Book where bookName={}".format(int(txt[0]))
    ...
    def search(self, way, index):
        ms = ConnDB(as_dict=False)
        if way == '书籍名称':
            sql = "select bookNo, bookName, publishName, bookType, bookPrice, bookStatus, studentNo from Book where bookName=%s"
            ...
            sql = "select bookNo, bookName, publishName, bookType, bookPrice, bookStatus, studentNo from Book where bookNo=%s" % index
            result = ms.ExecQuery(sql, ())
            ms.close()
            if len(result):
                columns = ("书籍编号", "书籍名称", "出版社名称", "类型", "价格", "状态", "持有人")
...
#收购帖界面及函数
class QuestFrame(Frame):  # 继承Frame类
    def __init__(self, master=None):
        Frame.__init__(self, master)
        self.root = master  # 定义内部变量root
        self.patient_id = StringVar()
    ...
    def createPage(self):
        Label(self, text='书籍收购帖', font='SimSum -16 bold').grid(row=0, column=1, pady=10)
...
#订单界面及函数
class ListFrame(Frame):  # 继承Frame类
    def __init__(self, master=None):
        Frame.__init__(self, master)
        self.root = master  # 定义内部变量root
        self.patient_id = StringVar()
    ...
    def createPage(self):
        Label(self, text='订单信息', font='SimSum -16 bold').grid(row=0, column=1, pady=10)
...
class EditFrame(Frame):
    ...
    def createPage(self):
        Label(self, text='修改个人信息', font='SimSum -16 bold').grid(row=0, column=1, pady=10)
        ttk.Label(self, text='  学    号: ', font='SimSun -14').grid(row=1, sticky=E, pady=10)
        ttk.Entry(self, textvariable=self.stuno).grid(row=1, column=1, sticky=W)
        ttk.Label(self, text='姓    名: ', font='SimSun -14').grid(row=2, sticky=E, pady=10)
        ttk.Entry(self, textvariable=self.stuname).grid(row=2, column=1, sticky=W)
        ttk.Label(self, text='性    别: ', font='SimSun -14').grid(row=3, sticky=E, pady=10)
        ttk.Entry(self, textvariable=self.sex).grid(row=3, column=1, sticky=W)
        ttk.Label(self, text='电    话: ', font='SimSun -14').grid(row=4, sticky=E, pady=10)
        ttk.Entry(self, textvariable=self.phoneno).grid(row=4, column=1, sticky=W)
        ttk.Label(self, text='密    码: ', font='SimSun -14').grid(row=5, sticky=E, pady=10)
        ttk.Entry(self, textvariable=self.password, show='*').grid(row=5, column=1, sticky=W)
        ttk.Button(self, text='保    存', width=30, bootstyle="primary-outline").grid(row=7, column=0,columnspan=2,pady=20)
 ...
 #         Button(self, text='更新', command=lambda: self.updateinfo(self.number.get()), font='SimSun -14', width=10).grid(row=6, column=1, pady=10)
